...
SLOT=0
KEYWORDS="~amd64"
IUSE="+gui cli debug +lv2 cpu_flags_x86_sse cpu_flags_x86_sse2 cpu_flags_x86_sse3 +libsamplerate +vst zita"
...
src_unpack() {
	unpack ${A}
	cd "${S}"
	epatch "${FILESDIR}/install-data-hook.patch"
}
...
src_configure() {
	...
	if use vst ; then
		myconf="${myconf} --with-vst-sources=${ROOT}usr/share/VST_SDK/VST2_SDK"
	fi
